...
```{r}
file_path = "C:/Users/jordi.catafal/Desktop/drive-download-20240515T062457Z-001/heart.csv"
...
summary(m1)
```
explicar que es pocho per R2 baix i que al saber que les nostres dades son rares probem amb >45 anys.
...
summary(m2)
```
No veiem gaire millora només amb age, probem amb totes les numèriques.
...
Veiem que dona un resultat força baix, però comparant amb només age tenim un molt millor resultat.
...
Només mirant numèriques del df2, surt 0.3 de R2, per tant amb totes les edats tenim un millor R2, però segueix sent molt baix, per aquest motiu anem a veure directament com afecten els factors.



probem métode stepwise amb els 2 casos.
...
mencionar ràpid que veiem una millora si utilitzem totes les variables tot i que mirant p-valors hi haurien categories que es podrien treure.
...
Per age > 45 trobem alguna variable amb més sentit tot i que la majoria segueixen sent contraries als valors lògics.
Upsloping good
flat meh
Downsloping bad

Comparant amb df veiem que hi ha algunes categories que tenim dades amb resultats que tenen mes sentit, com per exemple el trestbps que com major valor mes sa estas i per tant hauria de ser positiva en canvi hi han altres valors com el ca que en cap dels dos casos hauria de ser negatiu per almenys en el df te menor pes.

totes les variables que tinguin categories que tenen p-valor > 0.05 s'han de provar de optimitzar a veure com queden si les treim o si les deixem
...
Creiem que en el primer gràfic veiem aquest patró degut a que el target té valors que están distribuïts entre 0-0.2 i després fa un salt fins 0.6-1 aproximadament. I entre els valors 0.3-0.5 no hi ha valors de target.
...
summary(optimize2.1)
```
podem veure que el age efectivament no té gaire efecte
...
despres de fer totes aquestes proves veiem que lu millor es el que ens dona l'optimize i ens dona un major R2 si utilitzem el df2
mirant les grafiques de df, en el Q-Q residuals ens dona una molt bona grafica pero en la residuals vs fitted i scale location trobem patrons molt clars.
A les grafiques del df2, no sembla que hi hagin tants patrons pero aixo es degut a que te menys individus.
...
Veiem que empitjora si comparem amb el cas on no agafem els mout
...
En aquest cas tenim millors resultats si utilitzem els mout.
...
Lamdba es menor que 1 i major que 0, però al ser molt proper a 1 ns si la transformació de l'arrel cuadrada a la normal serà necessària, per tant probarem de fer-la i depend el resultat decidirem.
...
veiem que en els 2 casos on apliquem la transformació no ens ha donat bons resultats, per tant confirmem que al estar molt aprop de 1, el valor de lamda, no cal aquesta transformació.
...
```
```{r}
mB <- lm(formula= target ~ ., data =dfb45)

...
- Pattern Observed: Most residuals are centered around zero with a few points having high leverage and high standardized - Interpretation: This plot helps identify influential points that have a significant impact on the model’s fit. Points outside the Cook’s distance lines are potentially influential observations that may need further investigation.


From those graphics we can get this conclusions:
- Gap in Fitted Values: The gap in the first plot suggests that the target variable values are not continuous across the range but jump from 0.2 to 0.6, skipping intermediate values. This may indicate an issue with the distribution of the target variable.
...
# Dividir les dades en mostres de treball i test (80-20 split)
set.seed(123)  # Per assegurar la replicabilitat
sample_size <- floor(0.8 * nrow(dfb45))
train_index <- sample(seq_len(nrow(dfb45)), size = sample_size)
train_data <- dfb45[train_index, ]
test_data <- dfb45[-train_index, ]

# Seleccionar les variables explicatives i la variable resposta
# Variable resposta (Binary Target): target
# Variables explicatives: Seleccionem al menys dos variables numèriques
# En aquest exemple, utilitzarem les variables 'chol' (colesterol) i 'thalach' (màxim ritme cardíac assolit)

# Construir el model inicial de regressió logística
initial_model <- glm(target ~ chol + thalach, data = train_data, family = binomial)
summary(initial_model)
```
```{r}
# Selecció de factors més significatius basant-se en l'anàlisi multivariable anterior
# Per exemple, suposant que 'age' i 'trestbps' (pressió arterial en repòs) eren significatius
final_model <- glm(target ~ chol + thalach + age + trestbps + chol:thalach + age:chol, data = train_data, family = binomial)
